...
await Promise.all([
  ...
  build({ entryPoints: ['electron/pack-validation-worker.ts'], bundle: true, platform: 'node', format: 'esm', target: 'node22', outfile: 'dist-electron/pack-validation-worker.mjs', sourcemap: true }),
])