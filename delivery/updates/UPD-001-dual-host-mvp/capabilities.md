...
| UC-WEB-MIGRATE-LEGACY | web | User can open the update and retain a verified-MVP localStorage project through one transactional, recoverable migration. | `SC-WEB-LEGACY-MIGRATION` | specified |
| UC-WEB-RESTORE-SNAPSHOT | web | User can inspect and restore a known-good project snapshot without losing the pre-restore graph. | `SC-WEB-SNAPSHOT-RECOVERY` | specified |
| UC-WEB-RECOVER-STORAGE | web | User can respond to quota pressure by previewing safe cleanup or exporting a backup without silent project loss. | `SC-WEB-STORAGE-PRESSURE` | specified |
| UC-WEB-RESOLVE-SAVE-CONFLICT | web | User can resolve an older-tab revision conflict without silent overwrite. | `SC-WEB-CONCURRENT-EDIT`, `SC-WEB-PROJECT-LIBRARY` | specified |
...
| UC-DESKTOP-RESOLVE-EXTERNAL-CHANGE | desktop | User can resolve a project-folder change made after open without silently overwriting disk or in-memory work. | `SC-DESKTOP-PROJECT-FOLDER` | specified |
